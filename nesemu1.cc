...
const char* inputfn = "input.fmv";
const char* romFileName = "Super Dodge Ball (USA).nes";
...
namespace CPU /* CPU: Ricoh RP2A03 (based on MOS6502, almost the same as in Commodore 64) */
{
	...
	template<bool write> u8 MemAccess(u16 addr, u8 v)
	{
		...
		/**/ if (addr < 0x2000) { u8& r = RAM[addr & 0x7FF]; if (!write)return r; r = v; }
			...
			switch (addr & 0x1F)
			{
			case 0x14: // OAM DMA: Copy 256 NESbytes from RAM into PPU's sprite memory
...
{
	// Use the types defined in header, don't redefine them
	regtype reg;
	...
	sprite_info OAM2[8], OAM3[8];

	scrolltype scroll, vaddr;
...
{
	const u8 LengthCounters[32] = { 10,254,20, 2,40, 4,80, 6,160, 8,60,10,14,12,26,14,
									   12, 16,24,18,48,20,96,22,192,24,72,26,16,28,32,30 };
	const u16 NoisePeriods[16] = { 2,4,8,16,32,48,64,80,101,127,190,254,381,508,1017,2034 };
	const u16 DMCperiods[16] = { 428,380,340,320,286,254,226,214,190,160,142,128,106,84,72,54 };
...
int main(int/*argc*/, char** argv)
{
	...
	u8 ctrlNESbyte = fgetc(fp);
	u8 mappernum = fgetc(fp) | (ctrlNESbyte >> 4);
	...
	fclose(fp);
	printf("%u * 16kB ROM, %u * 8kB VROM, mapper %u, ctrlNESbyte %02X\n", rom16count, vrom8count, mappernum, ctrlNESbyte);
	...
	for (;;)
	{
		...
		if (SDB_TranslationTable[CPU::PC] != nullptr)
		{
			// Call the translated function
			SDB_TranslationTable[CPU::PC]();

			// Execute an RTS instruction
			// Pull the top two NESbytes off the stack
...
{
	// Call your existing main with dummy args
